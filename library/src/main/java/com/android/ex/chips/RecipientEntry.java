...
/* package */static final int INVALID_DESTINATION_TYPE = -1;
  ...
  /**
   * True when this entry is the first entry in a group, which should have a photo and display name,
   * while the second
  ...
  /**
   * Label of the destination which will be used when type was {@link Email#TYPE_CUSTOM}. Can be
   * null when {@link #mDestinationType} is {@link #INVALID_DESTINATION_TYPE}.
   */
  ...
  /**
   * This can be updated after this object being constructed, when the photo is fetched from remote
   * directories.
   */
  private byte[] mPhotoBytes;
  private final boolean mIsGalContact;

  private RecipientEntry(final int entryType, final String displayName, final String destination,
      final int destinationType, final String destinationLabel, final long contactId,
      final long dataId, final Uri photoThumbnailUri, final boolean isFirstLevel,
  ...
  /**
   * Construct a RecipientEntry from just an address that has been entered. This address has not
   * been resolved to a
  ...
  public static RecipientEntry constructFakeEntry(final String address, final boolean isValid) {
    ...
    return new RecipientEntry(ENTRY_TYPE_PERSON, tokenizedAddress, tokenizedAddress,
        INVALID_DESTINATION_TYPE, null, INVALID_CONTACT, INVALID_CONTACT, null, true, isValid, false /* isGalContact */);
  }
  ...
  public static RecipientEntry constructFakePhoneEntry(final String phoneNumber,
    ...
    return new RecipientEntry(ENTRY_TYPE_PERSON, phoneNumber, phoneNumber, INVALID_DESTINATION_TYPE,
        null, INVALID_CONTACT, INVALID_CONTACT, null, true, isValid, false /* isGalContact */);
  }

  /**
   * @return the display name for the entry. If the display name source is larger than {@link
   * DisplayNameSources#PHONE} we use the contact's display name, but if not, i.e. the display name
   * came from an email address or a phone number, we don't use it to avoid confusion and just use
   * the
   * destination instead.
   */
  ...
  /**
   * Construct a RecipientEntry from just an address that has been entered with both an associated
   * display name. This
   * address has not been resolved to a contact and therefore does not have a contact id or photo.
   */
  public static RecipientEntry constructGeneratedEntry(final String display, final String address,
      final boolean isValid) {
    return new RecipientEntry(ENTRY_TYPE_PERSON, display, address, INVALID_DESTINATION_TYPE, null,
        GENERATED_CONTACT, GENERATED_CONTACT, null, true, isValid, false /* isGalContact */);
  }
  ...
  public static RecipientEntry constructTopLevelEntry(final String displayName,
      ...
      final String destinationLabel, final long contactId, final long dataId,
      final Uri photoThumbnailUri, final boolean isValid, final boolean isGalContact) {
    return new RecipientEntry(ENTRY_TYPE_PERSON,
        pickDisplayName(displayNameSource, displayName, destination), destination, destinationType,
        destinationLabel, contactId, dataId, photoThumbnailUri, true, isValid, isGalContact);
  }

  public static RecipientEntry constructTopLevelEntry(final String displayName,
      final int displayNameSource, final String destination, final int destinationType,
      final String destinationLabel, final long contactId, final long dataId,
      final String thumbnailUriAsString, final boolean isValid, final boolean isGalContact) {
    return new RecipientEntry(ENTRY_TYPE_PERSON,
        pickDisplayName(displayNameSource, displayName, destination), destination, destinationType,
        destinationLabel, contactId, dataId,
        thumbnailUriAsString != null ? Uri.parse(thumbnailUriAsString) : null, true, isValid,
        isGalContact);
  }
  ...
  public static RecipientEntry constructSecondLevelEntry(final String displayName,
    ...
    return new RecipientEntry(ENTRY_TYPE_PERSON,
        pickDisplayName(displayNameSource, displayName, destination), destination, destinationType,
        destinationLabel, contactId, dataId,
        thumbnailUriAsString != null ? Uri.parse(thumbnailUriAsString) : null, false, isValid,
        isGalContact);
  }
