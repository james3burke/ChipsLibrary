...
public abstract class BaseRecipientAdapter extends BaseAdapter
  ...
  /**
   * The number of extra entries requested to allow for duplicates. Duplicates are removed from the
   * overall result.
   */
  ...
  /**
   * The "Waiting for more contacts" message will be displayed if search is not complete within this
   * many
   * milliseconds.
   */
  ...
  protected static class TemporaryEntry {
    ...
    public TemporaryEntry(final String displayName, final String destination,
        final int destinationType, final String destinationLabel, final long contactId,
        final long dataId, final String thumbnailUriString, final int displayNameSource,
  ...
  private static class DefaultFilterResult {
    ...
    public DefaultFilterResult(final List<RecipientEntry> entries,
        ...
        final List<RecipientEntry> nonAggregatedEntries, final Set<String> existingDestinations,
  ...
  private final class DefaultFilter extends Filter {
    @Override protected FilterResults performFiltering(final CharSequence constraint) {
        if (DEBUG) {
            Log.d(TAG, "start filtering. constraint: "
                + constraint
                + ", thread:"
        ...
        for (final RecipientEntry recipientEntry : mOriginalEntries) {
          final String constraintStr = constraint.toString().toLowerCase(Locale.getDefault());
            if (recipientEntry.getDisplayName()
                ...
                .contains(constraintStr) || recipientEntry.getDestination()
                .toLowerCase(Locale.getDefault())
      ...
      try {
        ...
        if (defaultDirectoryCursor == null) {
          if (DEBUG) Log.w(TAG, "null cursor returned for default Email filter query.");
        } else {
          ...
          final Set<String> existingDestinations = new HashSet<String>();
            ...
            {
                putOneEntry(new TemporaryEntry(defaultDirectoryCursor, false /* isGalContact */),
          ...
          final List<DirectorySearchParams> paramsList;
            if (limit > 0) {
                ...
                directoryCursor =
                    mContentResolver.query(DirectoryListQuery.URI, DirectoryListQuery.PROJECTION,
                        null, null, null);
          ...
          results.values =
              new DefaultFilterResult(entries, entryMap, nonAggregatedEntries, existingDestinations,
                  paramsList);
          results.count = 1;
        }
        ...
        if (defaultDirectoryCursor != null) defaultDirectoryCursor.close();
        if (directoryCursor != null) directoryCursor.close();
      }
      return results;
    }
    ...
    protected void publishResults(final CharSequence constraint, final FilterResults results) {
      ...
      if (results.values != null) {
        ...
        if (defaultFilterResult.paramsList != null) {
          final int limit =
              mPreferredMaxResultCount - defaultFilterResult.existingDestinations.size();
      ...
      if (TextUtils.equals(constraint, mCurrentConstraint)) {
        if (results.count > 0) {
          ...
          for (final TemporaryEntry tempEntry : tempEntries)
            putOneEntry(tempEntry, mParams.directoryId == Directory.DEFAULT, mEntryMap,
                mNonAggregatedEntries, mExistingDestinations);
        }
  ...
  /**
   * {@link #mEntries} is responsible for showing every result for this Adapter. To construct it, we
   * use {@link #mEntryMap}, {@link #mNonAggregatedEntries}, and {@link #mExistingDestinations}.
   * First, each destination (an email address or a phone number) with a valid contactId is inserted
   * into {@link #mEntryMap} and grouped by the contactId. Destinations without valid contactId
   * (possible if they aren't in
   ...
   * {@link #mEntries}. If
   * the number of destinations is not enough (i.e. less than {@link #mPreferredMaxResultCount}),
   * destinations in
  ...
  /**
   * Used to ignore asynchronous queries with a different constraint, which may happen when users
   * type characters
   * quickly.
   */
  ...
  /**
   * Handler specific for maintaining "Waiting for more contacts" message, which will be shown when:
   * - there are
   ...
   */
  @Override public void setAccount(final Account account) {
    mAccount = account;
  }
  ...
  /**
   * An extesion to {@link RecipientAlternatesAdapter#getMatchingRecipients} that allows additional
   * sources of
  ...
  public static List<DirectorySearchParams> setupOtherDirectories(final Context context,
      final Cursor directoryCursor, final Account account) {
    ...
    while (directoryCursor.moveToNext()) {
      ...
      // all local results.
      if (id == Directory.LOCAL_INVISIBLE) continue;
    ...
    if (preferredDirectory != null) paramsList.add(1, preferredDirectory);
    return paramsList;
  }
  ...
  protected void startSearchOtherDirectories(final CharSequence constraint,
    ...
    for (int i = 1; i < count; i++) {
      ...
      if (params.filter == null) params.filter = new DirectoryFilter(params);
  ...
  private static void putOneEntry(final TemporaryEntry entry, final boolean isAggregatedEntry,
      final LinkedHashMap<Long, List<RecipientEntry>> entryMap,
      final List<RecipientEntry> nonAggregatedEntries, final Set<String> existingDestinations) {
    if (existingDestinations.contains(entry.destination)) return;
    existingDestinations.add(entry.destination);
      if (!isAggregatedEntry) {
          nonAggregatedEntries.add(
              RecipientEntry.constructTopLevelEntry(entry.displayName, entry.displayNameSource,
                  entry.destination, entry.destinationType, entry.destinationLabel, entry.contactId,
                  entry.dataId, entry.thumbnailUriString, true, entry.isGalContact));
          ...
          entryList.add(
              RecipientEntry.constructSecondLevelEntry(entry.displayName, entry.displayNameSource,
                  entry.destination, entry.destinationType, entry.destinationLabel, entry.contactId,
                  entry.dataId, entry.thumbnailUriString, true, entry.isGalContact));
      } else {
  ...
  private List<RecipientEntry> constructEntryList(
    ...
    for (final Map.Entry<Long, List<RecipientEntry>> mapEntry : entryMap.entrySet()) {
      ...
      if (validEntryCount > mPreferredMaxResultCount) break;
    }
    ...
    if (photoThumbnailUri != null) {
      final byte[] photoBytes = mPhotoCacheMap.get(photoThumbnailUri);
        ...
        // notifyDataSetChanged() should be called by a caller.
            ...
            if (DEBUG) {
                Log.d(TAG,
                    "No photo cache for " + entry.getDisplayName() + ". Fetch one asynchronously");
            }
  ...
  private void fetchPhotoAsync(final RecipientEntry entry, final Uri photoThumbnailUri) {
    final AsyncTask<Void, Void, byte[]> photoLoadTask = new AsyncTask<Void, Void, byte[]>() {
      @Override protected byte[] doInBackground(final Void... params) {
        ...
        final Cursor photoCursor =
          ...
          if (photoCursor != null) {
              try {
                  if (photoCursor.moveToFirst()) return photoCursor.getBlob(PhotoQuery.PHOTO);
                  photoCursor.close();
              } finally {
                  photoCursor.close();
              }
          ...
          // an openFile call to the provider.
          {
              try {
                  ...
                  if (is != null) {
                      ...
                      try {
                          int size;
                          while ((size = is.read(buffer)) != -1) baos.write(buffer, 0, size);
                          is.close();
                      } finally {
                          is.close();
                      }
      ...
      @Override protected void onPostExecute(final byte[] photoBytes) {
        entry.setPhotoBytes(photoBytes);
        if (photoBytes != null) {
  ...
  public ArrayList<RecipientEntry> doQuery() {
    final Cursor cursor =
        mContentResolver.query(mQuery.getContentUri(), mQuery.getProjection(), null, null, null);
    final LinkedHashMap<Long, List<RecipientEntry>> entryMap =
        new LinkedHashMap<Long, List<RecipientEntry>>();
  ...
  private Cursor doQuery(final CharSequence constraint, final int limit, final Long directoryId) {
    ...
    final Cursor cursor =
        mContentResolver.query(uri, mQuery.getProjection(), selection, selectionArgs, null);
  ...
  @Override public RecipientEntry getItem(final int position) {
  ...
  public View getView(final int position, final View convertView, final ViewGroup parent) {
    ...
    if (TextUtils.isEmpty(displayName) || TextUtils.equals(displayName, destination)) {
      ...
      if (entry.isFirstLevel()) destination = null;
    }
    final View itemView =
    ...
    if (destinationTypeView != null) {
      final CharSequence destinationType =
          mQuery.getTypeLabel(mContext.getResources(), entry.getDestinationType(),
              entry.getDestinationLabel()).toString().toUpperCase(Locale.getDefault());
   ...
   * Each View must contain two TextViews (for display name and destination) and one ImageView (for
   * photo). Ids for
   * those should be available via {@link #getDisplayNameId()}, {@link #getDestinationId()}, and
   * {@link #getPhotoId()} .
   */
  ...
  /**
   * Returns a resource ID representing an image which should be shown when ther's no relevant photo
   * is available.
   */
  ...
  /**
   * Returns an id for TextView in an item View for showing a destination (an email address or a
   * phone number). By
  ...
  /**
   * Returns an id for TextView in an item View for showing the type of the destination. By default
   * {@link android.R.id#text2} is returned.
   */
